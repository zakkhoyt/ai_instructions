...
# ---- ---- ----  About this Script  ---- ---- ----
#
# Purpose: Configure AI instructions for various platforms (OVERHAUL VERSION)
# Author: Zakk Hoyt
# Usage: ./configure_ai_instructions_overhaul.zsh [OPTIONS]
#
# This is the OVERHAUL version implementing critical fixes from specification:
# - Menu always shows when --prompt set (removes conditional display)
# - Fixed number formatting for 10+ items (%2d -> %3d)
# - Xcode MCP only runs with explicit --mcp-xcode flag
# - Installation method displayed in menu header
# - Improved help text with examples
#
# For legacy version, see: configure_ai_instructions_NEW.zsh
#
...
function print_usage {
  ...
  slog_se "${i6}Manage instruction file installation"
  slog_se "${i6}  • Without --prompt: Auto-install all uninstalled files"
  slog_se "${i6}  • With --prompt: Show interactive menu for all files"
  slog_se "${i6}    (allows reviewing and modifying existing installations)"
  slog_se
  slog_se "${i4}" --code '--prompt' --default
  slog_se "${i6}Enable interactive menu for operations"
  slog_se "${i6}  • Shows menus even when no changes needed"
  slog_se "${i6}  • Allows reviewing current installation state"
  slog_se "${i6}  • Can modify existing installations"
  slog_se
  slog_se "${i4}" --code '--mcp-xcode' --default
  slog_se "${i6}Install Xcode MCP server configuration"
  slog_se "${i6}  • Only runs when explicitly requested"
  slog_se "${i6}  • Use with --prompt for interactive mode"
  slog_se
  ...
  slog_se
  slog_se "${i2}${SYMBOL_BULLET:-•} Review current installation state (interactive menu)"
  slog_se "${i4}" --code "./${script_name} --instructions --prompt" --default
  slog_se
  slog_se "${i2}${SYMBOL_BULLET:-•} Auto-install all missing files (no prompts)"
  slog_se "${i4}" --code "./${script_name} --instructions" --default
  slog_se
  slog_se "${i2}${SYMBOL_BULLET:-•} Configure Xcode MCP interactively"
  slog_se "${i4}" --code "./${script_name} --mcp-xcode --prompt" --default
  slog_se
  slog_se "${i2}${SYMBOL_BULLET:-•} Install instructions and Xcode MCP together"
  slog_se "${i4}" --code "./${script_name} --instructions --mcp-xcode --prompt" --default
  slog_se
  slog_se "${i2}${SYMBOL_BULLET:-•} Configure Claude with copy mode"
  slog_se "${i4}" --code "./${script_name} --ai-platform claude --configure-type copy --instructions" --default
  slog_se
  slog_se "${i2}${SYMBOL_BULLET:-•} Development workflow setup"
  slog_se "${i4}" --code "./${script_name} --dev-link --dev-vscode --workspace-settings" --default
  slog_se
  
  slog_se --bold "KEY IMPROVEMENTS (OVERHAUL VERSION)" --default
  slog_se
  slog_se "${i2}• Menu always shows with --prompt (even when all files installed)"
  slog_se "${i2}• Installation method displayed in menu header"
  slog_se "${i2}• Fixed number formatting for 10+ items"
  slog_se "${i2}• Xcode MCP only runs when explicitly requested"
  slog_se "${i2}• Clearer flag documentation with usage examples"
  slog_se
...
if [[ -n "${flag_instructions:-}" ]]; then
  # OVERHAUL FIX: Always show menu when --prompt is set, regardless of installation status
  # This allows users to review and modify existing installations
  
  typeset user_selection=""
  if [[ -n "${flag_prompt:-}" ]]; then
    # Show interactive menu even if all files are already installed
    slog_step_se --context info "Showing instruction file menu (--prompt requested)"
    user_selection=$(display_menu)
  else
    # Auto-install mode: only proceed if there are files to install
    if ! has_instructions_to_install; then
      slog_step_se --context success "All instruction files are already installed and current"
      exit 0
    fi
    user_selection="all"
  fi
